...
class Geminipy(object):
    ...
    def symbols(self):
        ...
        if r.status_code != 200:
            print("Error: getPrice returned an error")
            return null
        return r.content.decode()

    def price(self, symbol='btcusd', type='bid'):
        """Send a request for ticker price, return the response.
        
        Arguments:
        symbol -- currency symbol (default 'btcusd')
        type -- bid or ask price (default 'bid')
        """
        ...
        if r.status_code != 200:
            print("Error: getPrice returned an error")
            return null
        return json.loads(r.content)[type]
    ...
    def new_order(self, amount, price, side, client_order_id=None,
                  symbol='btcusd', type='exchange limit', options=["maker-or-cancel"]):
        """
